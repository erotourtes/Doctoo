...
export class DoctorService {
  ...
  async create(createDoctorDto: CreateDoctorDto) {
    ...
    if (candidateDoctor) throw new BadRequestException({ message: `Doctor associated with user with id ${user_id} already exists` })
    const createdDoctor = this.prisma.doctor.create({
      data: { user_id, about_me, payrate },
    });
    return plainToInstance(DoctorDto, createdDoctor);
  }

  async findMany() {
    const doctors = this.prisma.doctor.findMany({ include: {user: { select: { first_name: true, last_name: true } }} });
    return plainToInstance(DoctorDto, doctors)
  }

  async findById(id: string) {
    const doctor = await this.prisma.doctor.findUnique({ where: { id }, include: {user: { select: { first_name: true, last_name: true } }} })
  ...
  async update(id: string, updateDoctorDto: UpdateDoctorDto) {
    ...
    const updatedDoctor = await this.prisma.doctor.update({ where: { id }, data: { about_me, payrate }, include: {user: { select: { first_name: true, last_name: true } }} })
  ...
  async remove(id: string) {
    ...
    if (!deleteResult) throw new InternalServerErrorException({ message: `Something went wrong during deleting doctor with id ${id}` })
