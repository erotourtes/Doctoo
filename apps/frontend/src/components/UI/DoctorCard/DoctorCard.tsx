...
function ImageWithFavorite({ url, isFavorite, onClickFavorite }: ImageWithFavoriteProps) {
  return (
    <div className='group relative col-span-1 row-span-3 aspect-square h-[80px] sm:row-span-4 sm:h-[112px]'>
      ...
      <div className='absolute left-0 top-0 flex h-full w-full items-end justify-end rounded-lg bg-semi-transparent opacity-0 transition duration-300 group-hover:opacity-100'>
        <button className='mb-1 mr-1 rounded-lg bg-white hover:bg-grey-5' onClick={onClickFavorite}>
...
function Rating({ rating, reviewsCount, starClassName }: RatingProps) {
  const getRatingStars = () => {
    const stars = [];
    for (let i = 0; i <= 4; i++) {
      if (rating > i + 0.5) stars.push(<Icon variant='star' className={cn('text-main', starClassName)} />);
      else if (rating >= i) stars.push(<Icon variant='star-half' className={cn('text-main', starClassName)} />);
      else stars.push(<Icon variant='star' className={cn('text-grey-3', starClassName)} />);
    }
    return stars;
  };
  return (
  ...
  return (
    <div className='col-span-full col-start-1 mt-1 md:row-auto lg:col-start-3 lg:row-start-2 lg:row-end-4 lg:mt-0'>
      <div className={`grid ${timestamps.length > 1 ? 'grid-cols-2 grid-rows-2' : ''} gap-2 whitespace-nowrap`}>
        {timestamps.slice(0, 3).map((timestamp, i) => (
          <Button
            key={i}
            type='secondary'
